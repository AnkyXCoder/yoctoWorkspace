...
while [[ $# -gt 0 ]]; do
	...
	--machine)
		shift
		if [[ -z "${1:-}" ]]; then
			echo "ERROR: --machine requires an argument"
			exit 1
		fi
		MACHINE="$1"
		shift
...
clone_with_branch "sources/poky" "${POKY_BRANCH}" "${POKY_REPO}"
...
export POKY_DIR="\${YOCTO_WORKSPACE}/sources/poky"
...
echo "load the build environment with:"
